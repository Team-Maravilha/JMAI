...
class MYPDF extends TCPDF
{
    ...
    public function Header()
    {
        ...
        $this->SetY($this->GetY() + 3);
        $this->Cell(0, 0, 'Data de Submissão: ' . $informacao_requerimento['data_criacao'], 0, false, 'R', 0, '', 0, false, 'T', 'M');
    }
...
$pdf->SetX(5);
$pdf->Cell(0, 0, 'Comprovativo Requerimento de Atestado Médico de Incapacidade', 0, false, 'L', 0, '', 0, false, 'T', 'M');
$pdf->setFont('helvetica', '', 9);
$pdf->SetY($pdf->GetY() + 7);
$pdf->SetX(5);
$pdf->MultiCell(0, 0, 'O comprovativo de requerimento de atestado médico de incapacidade é emitido pela plataforma informática da Junta Médica de Avaliação de Incapacidades (JMAI) e tem como objetivo comprovar o preechimento do requerimento de atestado médico de incapacidade.', 0, 'L', 0, 1, '', '', true, 0, false, true, 0, 'M', false);

$pdf->SetY($pdf->GetY() + 5);
//TABLE WITH 1 ROW AND 2 CELLS
$pdf->MultiCell(0, 0, 'Ao dia ' . date('d/m/Y') . ' o requerimento de atestado médico de incapacidade com o número ' . $informacao_requerimento['numero_requerimento'] . ' encontra-se na seguinte situação: ' . $informacao_requerimento['texto_estado'] . '.', 0, 'L', 0, 1, '', '', true, 0, false, true, 0, 'M', false);
...
$pdf->Ln(10);
$texto = 'O presente documento visa a comprovar que no dia ' . $informacao_requerimento['data_criacao'] . ' foi submetido por ' . $informacao_requerimento['informacao_utente']['nome'] . ', um requerimento de atestado médico de incapacidade na plataforma informática da Junta Médica de Avaliação de Incapacidades (JMAI), com o número ' . $informacao_requerimento['numero_requerimento'] . '.';
$pdf->MultiCell(0, 0, $texto, 0, 'J', 0, 1, '', '', true, 0, false, true, 0, 'M', false);
